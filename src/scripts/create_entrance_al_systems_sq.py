"""Build a normalized JSONL file from the Albanian systems entrance exam dataset."""
...
DATASET_ID = "marjpri/albanian-medical-exams-systems-mcq-400"
DATASET_NAME = "entrance-al-systems-sq"
OUTPUT_PATH = Path("data/processed/entrance_al_systems_sq.jsonl")
DEMO_OUTPUT_PATH = Path("data/processed/entrance_al_systems_sq_demo.jsonl")
...
def main() -> None:
    ...
    try:
        ...
        records.append(
            DatasetRecord(
                id="entrance-al-systems-sq-demo-1",
                language="sq",
                country="AL",
                ...
                split="demo",
                question="Sistemi i qarkullimit të gjakut përbëhet nga:",
                options=("zemra dhe enët e gjakut", "veshkat dhe ureterët", "truri dhe palca", "mëlçia dhe pankreasi"),
                label="zemra dhe enët e gjakut",
                specialty="biology systems",
    ...
    for row in dataset:
        ...
        records.append(
            DatasetRecord(
                id=f"entrance-al-systems-sq-{row.get('original_question_num', len(records))}",
                language="sq",
                country="AL",
                dataset_name=DATASET_NAME,
                task=MEDICAL_ENTRANCE_MCQ.name,
