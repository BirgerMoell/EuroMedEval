"""Build a normalized JSONL file from Swedish emergency medicine cases."""
...
def main() -> None:
    """Write the migrated Swedish emergency medicine dataset to JSONL."""
    ...
    for raw_id, row in raw_data.items():
        ...
        records.append(
            DatasetRecord(
                id=f"se-em-sv-{raw_id}",
                language="sv",
                country="SE",
                dataset_name="se-em-sv",
    ...
    output_path = Path("data/processed/se_em_sv.jsonl")
