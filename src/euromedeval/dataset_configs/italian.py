...
ITALIAN_DATASET_CONFIGS = [
    ...
    DatasetConfig(
        name="miur-clinica-it",
        pretty_name="MIUR Medical State Exam (Clinical Area)",
        language="it",
        country="IT",
        task=CLINICAL_CASE_MCQ.name,
        tier=DatasetTier.GOLD,
        status=DatasetStatus.OFFICIAL,
        source_url="https://huggingface.co/datasets/Detsutut/miur-medicina-clinica",
        source_type="physician state exam clinical-area MCQ",
        license="Apache-2.0",
        access_mode=AccessMode.PERMISSIONED,
        native=True,
        clinically_reviewed=False,
        creation_script="src/scripts/create_miur_clinica_it.py",
        description="Official MIUR clinical-area question bank for the Italian physician state examination.",
        notes="Dataset is gated behind a usage acknowledgment on Hugging Face; keep as a permissioned native benchmark.",
    ),
    DatasetConfig(
        name="miur-preclinica-it",
        pretty_name="MIUR Medical State Exam (Pre-Clinical Area)",
        language="it",
        country="IT",
        task=MEDICAL_KNOWLEDGE_MCQ.name,
        tier=DatasetTier.GOLD,
        status=DatasetStatus.OFFICIAL,
        source_url="https://huggingface.co/datasets/Detsutut/miur-medicina-preclinica",
        source_type="physician state exam pre-clinical MCQ",
        license="Apache-2.0",
        access_mode=AccessMode.PERMISSIONED,
        native=True,
        clinically_reviewed=False,
        creation_script="src/scripts/create_miur_preclinica_it.py",
        description="Official MIUR pre-clinical question bank for the Italian physician state examination.",
        notes="Dataset is gated behind a usage acknowledgment on Hugging Face; keep as a permissioned native benchmark.",
    ),
    DatasetConfig(
        name="medexpqa-it",
